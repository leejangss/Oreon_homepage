/* OREON IR BOARD - Imweb custom renderer v3
...
(function () {
  ...
  const STYLE_ID = "oreon-ir-board-v3-style";
...
@media(max-width:900px){
  ...
  .oreon-ir-contact-v3{margin-top:46px;padding:36px 28px;flex-direction:column;align-items:flex-start}
  .oreon-ir-contact-v3 a{width:100%;box-sizing:border-box}
}
...
`;
  ...
  function extractPosts(root){
    ...
    for(const a of anchors){
      ...
      let title = norm(a.textContent);
      if(!title || EXCLUDE.has(upper(title))) continue;

      // Avoid accidentally selecting utility links inside a post row.
      if(/^(이동|수정|지우기|삭제|공유|인쇄)$/.test(title)) continue;
    ...
    // Preserve DOM order, but if numbers exist, use descending number like the native board.
  ...
  function build(root){
    if(root.dataset.oreonIrV3Building === "1") return;
    root.dataset.oreonIrV3Building = "1";
    ...
    // CTA
    const existingCTA = document.querySelector('.oreon-ir-contact-v3[data-for="'+ID+'"]');
    if(!existingCTA){
      const cta = document.createElement("div");
      cta.className = "oreon-ir-contact-v3";
      cta.dataset.for = ID;
      cta.innerHTML = `
        <div>
    ...
    root.dataset.oreonIrV3Building = "0";
  }
  ...
  function boot(tries){
    ...
    const obs = new MutationObserver(muts=>{
      const changedNative = muts.some(m=>{
        ...
        return t && !t.closest(".oreon-ir-shell") && !t.closest(".oreon-ir-contact-v3");
      });
